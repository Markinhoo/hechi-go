-- Negative spell entries may describe damage inflicted on OTHER houses.
-- Reconcile recorded earnings with the student's actual balance instead.
-- This corrects existing displays without changing balances or spell history.
-- House totals remain independent: some spells only affect entire houses.
begin;
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'casaId', a.casa_id,
    'puntos', a.puntos,
    'puntosPositivos', greatest(coalesce(p.resumen_positivos, 0), a.puntos, 0),
    'puntosNegativos', greatest(coalesce(p.resumen_positivos, 0), a.puntos, 0) - a.puntos,
    'cartas', a.cartas,
