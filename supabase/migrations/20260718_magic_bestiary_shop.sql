-- Magic Bestiary: gold economy and purchasable creature collection.

alter table hechi.alumnos
  add column if not exists galeones integer not null default 0,
  add column if not exists bestiario jsonb not null default '[]'::jsonb;
