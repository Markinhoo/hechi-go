begin;
alter table hechi.cartas_arena add column if not exists tipo text not null default 'criatura' check(tipo in('criatura','magia','trampa'));
insert into hechi.cartas_arena(id,ataque,defensa,copias,afinidades,tipo) values
('hechizo-incendio',0,0,4,array[]::text[],'magia'),
('hechizo-engorgio',0,0,4,array[]::text[],'magia'),
('hechizo-elixir',0,0,2,array[]::text[],'magia'),
('hechizo-patronus',0,0,1,array[]::text[],'trampa'),
('hechizo-confundo',0,0,2,array[]::text[],'trampa'),
('hechizo-invisibilidad',0,0,1,array[]::text[],'trampa')
on conflict(id) do update set copias=excluded.copias,tipo=excluded.tipo;
...
returns jsonb language sql immutable as $$
 ...
 'apoyos',(select jsonb_agg(case when value='null'::jsonb then value when p_propio then value else '{"oculta":true}'::jsonb end order by ord) from jsonb_array_elements(coalesce(p_jugador->'apoyos','[null,null,null]'::jsonb)) with ordinality e(value,ord)),
...
begin
 ...
 if not coalesce(p_aceptar,false) then
  ...
  a:=hechi.arena_rellenar(jsonb_build_object('vida',4000,'mano','[]'::jsonb,'apoyos','[null,null,null]'::jsonb,'campo','[null,null,null,null,null]'::jsonb,'mazo',hechi.arena_mazo()));
  b:=hechi.arena_rellenar(jsonb_build_object('vida',4000,'mano','[]'::jsonb,'apoyos','[null,null,null]'::jsonb,'campo','[null,null,null,null,null]'::jsonb,'mazo',hechi.arena_mazo()));
...
begin
 ...
 j:=j||jsonb_build_object('apoyos',coalesce(j->'apoyos','[null,null,null]'::jsonb));
 r:=r||jsonb_build_object('apoyos',coalesce(r->'apoyos','[null,null,null]'::jsonb));
 if p_accion='rendirse' then
  ...
  if p_accion='invocar' then
   ...
   if criatura='hechizo-engorgio' then
    ...
    casilla:=(p_datos->>'casilla')::integer;
    if casilla is null or casilla<0 or casilla>2 or j->'apoyos'->casilla<>'null'::jsonb then raise exception 'Elige un espacio de magia o trampa libre'; end if;
