-- Multijugos sets the caster's total to the selected student's exact balance.
begin;

-- The difference can exceed 100 when copying an unrestricted student balance.
...
alter table hechi.participaciones add constraint participaciones_puntos_check
  check (puntos between -100 and 100 or titulo = 'Ajuste manual' or carta = 26);
...
begin
  ...
  select * into v_objetivo from hechi.alumnos where id = p_objetivo_id and clase_id = v_clase.id for update;
  if not found then
  ...
  -- Copy the exact balance, including zero/negative totals; multipliers do not apply.
  v_puntos := v_objetivo.puntos - v_alumno.puntos;

  update hechi.alumnos
  set oportunidades = oportunidades - 1,
      cartas = array_append(cartas, p_numero),
      puntos = v_objetivo.puntos,
      updated_at = now()
  where id = v_alumno.id;

  v_positivos := coalesce((v_clase.puntajes_positivos->>v_alumno.casa_id)::integer, coalesce((v_clase.puntajes->>v_alumno.casa_id)::integer, 0)) + greatest(v_puntos, 0);
  v_negativos := coalesce((v_clase.puntajes_negativos->>v_alumno.casa_id)::integer, 0) + greatest(-v_puntos, 0);
