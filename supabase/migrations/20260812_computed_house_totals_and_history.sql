-- Compute house totals from positive/negative breakdown and expose a longer spell history.

update hechi.clases
set puntajes = jsonb_build_object(
      'gryffindor', coalesce((puntajes_positivos->>'gryffindor')::integer, 0) - coalesce((puntajes_negativos->>'gryffindor')::integer, 0),
      'slytherin', coalesce((puntajes_positivos->>'slytherin')::integer, 0) - coalesce((puntajes_negativos->>'slytherin')::integer, 0),
      'ravenclaw', coalesce((puntajes_positivos->>'ravenclaw')::integer, 0) - coalesce((puntajes_negativos->>'ravenclaw')::integer, 0),
      'hufflepuff', coalesce((puntajes_positivos->>'hufflepuff')::integer, 0) - coalesce((puntajes_negativos->>'hufflepuff')::integer, 0)
    ),
    updated_at = now();
