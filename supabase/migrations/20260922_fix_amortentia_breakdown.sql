-- Update both students, both house breakdowns and both histories atomically.
...
begin
  ...
  if not found or v_alumno.password is distinct from p_password then
  ...
  values (v_clase.id, v_alumno.id, p_numero, v_puntos, v_alumno.casa_id, p_titulo, p_descripcion);

  insert into hechi.participaciones(clase_id, alumno_id, carta, puntos, casa_objetivo, titulo, descripcion)
  values (v_clase.id, v_companero.id, p_numero, v_puntos, v_companero.casa_id, p_titulo, 'Amortentia compartida por ' || v_alumno.nombre);
